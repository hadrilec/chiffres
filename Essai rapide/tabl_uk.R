tabl_uk<-function(dig=1){
  ...
  base_old<<-NULL
  row1 <-  ligne_date()
  row2 <-f_row("<b>Produit interieur brut</b>","uk.pib_vol_q",VT,VA,base,dig=dig)
  ...
  # row8 <-f_row("Demande interieure hors stocks","us.di_contrib",NIV,NIV_AN,base)
  # row9 <-f_row("Variation de stocks","us.stocks.contrib",NIV,NIV_AN,base)
  # row10 <-f_row("Commerce exterieur","us.comext.contrib",NIV,NIV_AN,base)
  



tabl<-paste("<h1>Royaume-Uni</h1>",version(base),"<table style=\"width:100%\" border=1>",row1,row2,row3,row4,row5,row6,row7,"</table>",sep="")
tabl2<-gsub("&nbsp;%","",tabl)
tabl3<-gsub("\\+","",tabl2)
tabl4<-gsub("\\**","",tabl3)
tabl5<-gsub(">~~","><s>",tabl4)
